...
class checkpoint():
    def __init__(self, args):
        self.args = args  # args
        self.log = torch.Tensor()  # log
        now = datetime.datetime.now().strftime('%Y-%m-%d-%H:%M:%S')
        # print(now)  # 2020-03-01-17:02:50

        # load/save
        if args.load == '':
            if args.save == '':
                args.save = now
            self.dir = './MGN/output/' + args.save  # dir about XXX
        else:
            self.dir = './MGN/output/' + args.load  # dir about XXX
        ...
        if args.reset:
            os.system('rmdir /s /q ' + self.dir)  # 将 rm -rf 改为 Windows 命令
            args.load = ''
        ...
        self.log_file = open(self.dir + '/log.txt', open_type)  # log_file  写些训练过程的信息
        with open(self.dir + '/config.txt', open_type) as f:  # 训练的参数信息
            f.write(now + '\n\n')
