from .common import list_pictures  # 改为相对导入
...
class Market1501(dataset.Dataset):
    def __init__(self, args, transform, dtype):
        ...
        if dtype == 'train':
            data_path = data_path.replace('/', '\\') + '\\bounding_box_train'
        elif dtype == 'test':
            data_path = data_path.replace('/', '\\') + '\\bounding_box_test'
        else:
            data_path = data_path.replace('/', '\\') + '\\query'
...
# 5） 01 表示 c1s1_001051 这一帧上的第1个检测框，由于采用DPM检测器，对于每一帧上的行人可能会框出好几个bbox。00 表示手工标注框
    ...
    def id(file_path):  # 返回每个人的标签编号，从0001到1501
        """
        :param file_path: windows style file path
        :return: person id
        """
        return int(file_path.split('\\')[-1].split('_')[0])  # 使用Windows路径分隔符
    ...
    def camera(file_path):  # 返回摄像头的id
        ...
        :return: camera id
        """
        return int(file_path.split('\\')[-1].split('_')[1][1])  # 使用Windows路径分隔符
